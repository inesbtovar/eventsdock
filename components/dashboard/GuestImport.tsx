...
type Mode = 'idle' | 'add' | 'mapping' | 'importing'
...
export default function GuestImport({ eventId }: Props) {
  ...
  function reset() {
    ...
    setParsed(null); setColName(''); setColEmail(''); setColPhone('')
    setColDietary(''); setColPlusOne('')
    setMode('idle')
    if (fileRef.current) fileRef.current.value = ''
  }

  // Step 1: read the file in the browser and extract headers + rows
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Auto-select columns if obvious matches exist
      const norm = (s: string) => s.toLowerCase().replace(/[\s_\-\.]/g, '')
      const nameGuess = headers.find(h => ['name','nome','fullname','nomecompleto','guest','convidado'].includes(norm(h))) ?? ''
      const emailGuess = headers.find(h => ['email','e-mail','mail','correio'].includes(norm(h))) ?? ''
      const phoneGuess = headers.find(h => ['phone','tel','telefone','telemovel','mobile','celular','whatsapp'].includes(norm(h))) ?? ''
      const dietaryGuess = headers.find(h => ['dietary','dieta','restricoes','alimentacao','food'].includes(norm(h))) ?? ''
      const plusGuess = headers.find(h => ['plusone','plus','acompanhante','guest2'].includes(norm(h))) ?? ''

      setColName(nameGuess)
      setColEmail(emailGuess)
      setColPhone(phoneGuess)
      setColDietary(dietaryGuess)
      setColPlusOne(plusGuess)
  ...
  // Step 2: user confirmed mappings, send to API
  async function handleImport() {
    ...
    try {
      // Build the guest list from the mapped columns
      ...
      if (guests.length === 0) {
        toast.error('No guests found in the selected column. Please check your mapping.')
        setMode('mapping')
        setImporting(false)
        return
      }
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ padding: '20px 24px', borderBottom: '1px solid #F5F0E8', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h3 style={{ margin: 0, fontWeight: '600', color: '#2D2016', fontSize: '16px' }}>
                Match your columns
              </h3>
              <p style={{ margin: '3px 0 0', fontSize: '12px', color: '#A08060' }}>
                {parsed.fileName} · {parsed.rows.length} rows found
              </p>
            </div>
          ...
          <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <p style={{ margin: 0, fontSize: '13px', color: '#7A6652', lineHeight: '1.5' }}>
              We found <strong>{parsed.headers.length} columns</strong> in your file. Tell us which one is which:
            </p>
