...
import GuestTable from '@/components/dashboard/GuestImport'
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <nav className="bg-white border-b border-stone-200 px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          ...
          <h1 className="text-stone-800 font-semibold">{event.name}</h1>
          <span className={`ml-auto text-xs px-2 py-1 rounded-full font-medium ${
            event.is_published ? 'bg-green-100 text-green-700' : 'bg-stone-100 text-stone-500'
          }`}>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Guests', value: total, color: 'bg-white' },
            { label: 'Confirmed', value: confirmed, color: 'bg-green-50' },
            { label: 'Declined', value: declined, color: 'bg-red-50' },
            { label: 'Pending', value: pending, color: 'bg-yellow-50' },
          ].map(stat => (
            <div key={stat.label} className={`${stat.color} rounded-xl border border-stone-200 p-5`}>
